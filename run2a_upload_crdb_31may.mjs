...
const snapResp = await fetch(`${BASE}/api/arrears-snapshots`, {
  ...
  body: JSON.stringify({
    ...
    created_by: 'catch-up-run2a-crdb-31may',
    notes: 'Run 2A — BANK CRDB full 31.05; arrears≤2026-05-31',
  }),
});
...
const idem = 'run2a-crdb-31may-retry1-' + crypto.createHash('sha256').update(idemSrc).digest('hex').slice(0, 16);

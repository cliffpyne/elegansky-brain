// CRDB upload — June 3 16:00 EAT → midnight EAT window.
...
const SINCE_ISO = process.argv[4] || '2026-06-03T13:00:00Z'; // 16:00 EAT
const UNTIL_ISO = process.argv[5] || '2026-06-03T21:00:00Z'; // 00:00 EAT next day
const CONFIRM = process.argv.includes('--confirm');

const CHANNEL = 'bank';
const REF_SUFFIX = 'B';
const SHEET_ID = '1rdSRNLdZPT5xXLRgV7wSn1beYwWZp41ZpYoLkbGmt0o';
const SHEET_TAB = 'PASSED';
const DEPOSIT_ACCT = '785';
const SAASANT_CSV = `/home/clifforddennis/Downloads/BRAIN_unused_NEEDS_SAASANT_04jun_crdb.csv`;
...
const idemKey = `heisenberg-04jun-crdb-${Date.now()}`;
...
const snap = await db.query(
  ...
  [AS_OF, JSON.stringify(arrears), arrears.length, arrears.reduce((s,a)=>s+Number(a.invoiceBalance||a.balance||0),0).toFixed(2), 'heisenberg-script', 'CRDB 16:00 to 00:00 EAT manual upload']
);
